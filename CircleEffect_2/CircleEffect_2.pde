    ...
    for(int y = 0; y < height + step; y += step) {
      ...
      for(int r = 0; r < circles.length && circles[r] >=0 && !found ; r++) {
        ...
        if(distance < 0 && distance > - colorLimit) {
          ...
          int green = 255 - (int) map(- distance, 0, colorLimit, 0, 180);
          int ignoreValue = round(map(- distance, 0, colorLimit, 0, 5));
          boolean ignore = false;
          switch(ignoreValue) {
            case 3 : if(random(10) > 9) ignore = true;
              break;
            case 4 : if(random(10) > 7) ignore = true;
              break;
            case 5 : if(random(10) > 3) ignore = true;
              break;
          }
          if(!ignore)
            c = color(0,green,0);
